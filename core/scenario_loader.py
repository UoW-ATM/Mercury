...
class ScenarioLoaderSelector:
	def __init__(self):
		self.available_loaders = {'ScenarioLoaderStandard': ScenarioLoaderStandard,
						'ScenarioLoaderSimple': ScenarioLoaderSimple,
...
class ScenarioLoaderSimple(ScenarioLoader):
	...
	def load_all_data(self, data_to_load=[], connection=None, process=None,
		...
		if not connection is None and connection['type']=='mysql':
...
class ScenarioLoaderStandard(ScenarioLoader):
	"""
	This loader is used to load things from csv files or the database following
	the format specified in the paras file.
	"""

	# This is a list of the tables required to run the scenario loader.
	# The names correspond to how their variable name in the paras file
	required_tables = ['input_scenario',
					  'input_delay_paras',
					  'input_aircraft_eq_badacomputed',
					  'input_schedules',
					  'input_subset',
					  'input_atfm_delay',
					  'input_atfm_prob',
					  'input_regulation_at_airport_days',
					  'input_route_pool',
					  'input_route_pool_has_airspace',
					  'input_airspace',
					  'input_trajectory_pool',
					  'input_fp_pool',
					  'input_fp_pool_point',
					  'input_airport_curfew',
					  'input_curfew_extra_time'
					  'input_airports_with_curfews',
					  'input_airports_curfew2',
					  'input_curfew_extra_time',
					  'input_airport',
					  'input_taxi_in',
					  'input_taxi_out',
					  ...
					  'input_doc',
					  'input_non_pax_cost',
					  'input_non_pax_cost_fit',
					  'input_cost_curfews',
					  'input_estimated_cost_curfews',
					  'input_airline',
					  'input_extra_cruise_if_dci',
					  'input_flight_uncertainties',
					  'input_soft_cost',
					  'input_itinerary',
					  'input_atfm_regulation_at_airport_manual',
					  'input_atfm_regulation_at_airport'
					  ]
	...
	def create_flight_plans(self):
		...
		for i, row in self.df_flight_plan_pool.iterrows():
			...
			self.dict_fp[(origin, destination, bada_code_ac_model, trajectory_pool_id, route_pool_id)] = fp

	def load_all_data(self, rs=None, connection=None, profile_paras={}, **garbage):
		print('ScenarioLoaderStandard')
		dir_name = '_'.join([str(model_version),str(self.paras['scenario'])])
		if 'path' in profile_paras:
			path = Path(profile_paras['path'])

			if path.exists() and profile_paras['type']=='parquet':
				print('Loading all data as parquet with ScenarioLoaderSimple from', path)
				for k in self.paras:
					if 'input_' in k:
						self.paras[k] = 'read_parquet(\''+str(path)+'/'+self.paras[k]+'.parquet\')'
		...
		while reading:
			try:
				self.df_flight_plan_pool = read_fp_pool(connection,
														fp_pool_table=self.paras['input_fp_pool'],
														fp_pool_point_table=self.paras['input_fp_pool_point'],
	...
	def load_scenario(self, connection=None):
		...
		my_module = importlib.import_module('Mercury.data_generation.scenarios_{}'.format(self.paras['project']))
		scenarios = getattr(my_module, 'scenarios')
		columns = getattr(my_module, 'scenarios')

		for col in columns:
			if not col in ['id', 'scenario']:
				setattr(self, col, df[col].iloc[0])

		# self.FP = df.FP.iloc[0]
		# self.FAC = df.FAC.iloc[0]
		# self.TA = df['4DTA'].iloc[0]
		# self.uptake = df['uptake'].iloc[0]
		# self.regulations_airport_day = df['regulations_airport_day'].iloc[0]
		# self.manual_airport_regulations = df['manual_airport_regulations'].iloc[0]
		# self.stochastic_airport_regulations = df['stochastic_airport_regulations'].iloc[0]
		# self.delays = df['delays'].iloc[0]

		df_scn_paras = read_scenario_paras(connection,
											scenario_table=self.paras['input_scenario'],
											delay_paras_table=self.paras['input_delay_paras'],
											scenario=self.paras['scenario'])
	...
	def load_atfm_regulations(self, connection=None):
		...
		self.non_weather_atfm_delay_dist = read_iedf_atfm(connection,
									table=self.paras['input_atfm_delay'],
									where = "WHERE atfm_type='non_weather"+post_fix+" AND scenario=\'"+self.delays+"\'",
									scipy_distr=True)

		self.non_weather_prob_atfm = read_prob_atfm(connection,
									where = "WHERE atfm_type='non_weather"+post_fix+" AND scenario=\'"+self.delays+"\'",
									table=self.paras['input_atfm_prob'])

		self.weather_atfm_delay_dist = read_iedf_atfm(connection,
									table=self.paras['input_atfm_delay'],
									where = "WHERE atfm_type='weather"+post_fix+" AND scenario=\'"+self.delays+"\'",
									scipy_distr=True)
	...
	def load_days_possible_regulation_at_airports(self, connection=None):
		self.dregs_airports_days = read_ATFM_at_airports_days(connection,
									regulation_at_airport_days_table=self.paras['input_regulation_at_airport_days'])
	...
	def load_curfews(self, connection=None):
		self.dict_cf = {}
		if self.paras['input_airport_curfew'] is not None:
			df_cf = read_airports_curfew_data(connection=connection,
											airport_table=self.paras['input_airport_curfew'],
											icao_airport_name=self.paras['icao_airport_name'],
											curfew_airport_name=self.paras['curfew_airport_name'],
											curfews_db_table=self.paras['input_airports_with_curfews'],
											curfews_db_table2=self.paras['input_airports_curfew2'],
											airports=self.airport_list,
											only_ECAC=True,
											curfew_extra_time_table=self.paras['input_curfew_extra_time'],
											airport_info_table=self.paras['input_airport'])
	...
	def load_cost_data(self, connection=None):
		...
		df_np_cost_fit = read_non_pax_cost_fit_data(connection,
													table=self.paras['input_non_pax_cost_fit'])

		df_np_cost_fit = df_np_cost_fit.set_index(['scenario', 'phase'])
		...
		self.dict_curfew_nonpax_costs = read_nonpax_cost_curfews(connection,
															curfew_cost_table=self.paras['input_cost_curfews'])

		self.dict_curfew_estimated_pax_avg_costs = read_estimated_avg_costs_curfews(connection,
																				curfew_estimated_avg_table=self.paras['input_estimated_cost_curfews'])
	...
	def load_airline_data(self, connection):
		self.df_airlines_data = read_airlines_data(connection,
												airline_table=self.paras['input_airline'],
	...
	def load_pax_data(self, connection):
		self.df_pax_data = read_itineraries_data(connection,
												table=self.paras['input_itinerary'],
												#flights=list(self.flights.keys()))
...
class ScenarioLoaderStandardLocal(ScenarioLoaderStandard):
	"""
