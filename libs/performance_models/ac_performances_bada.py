...
from Mercury.libs.performance_tools import trajectory as tr
from Mercury.libs.performance_tools.ac_performances import AircraftPerformance as AircraftPerformanceGeneric


class AircraftPerformanceBADA(AircraftPerformanceGeneric):
    engine_type = ""
    model_version = -1
    ...
    def _estimate_avg_cruise_weight(self, climb_kg, descent_kg, cruise_m, cruise_dist, cruise_time, fl, payload_lf=0.8,
                                    force=False, cruise_wind_kt=0, cruise_nom_kt=0, cruise_ground_kt=0):
        ct = self._trajectory_cruise_estimation_with_fl_change(fl, self.oew + descent_kg + self.mpl * payload_lf,
    ...
    def _set_climb_descent_fuel_flow_performances(self, fl, climb_ff, descent_ff):
        """
        Add the fl, climb fuel flow and descent fuel flow arrays in the flight performance object
    ...
    def _compute_sr(self, fl, mass, m, cruise_wind=0, bank=0):
        """
    ...
    def estimate_climb_fuel_flow(self, from_fl, to_fl, time_climb=None, planned_avg_speed_kt=None):
        ...
        """
        return self._avg_climb_descent_ff(self.fl_climb_descent, self.climb_ff, from_fl, to_fl)
    ...
    def estimate_descent_fuel_flow(self, from_fl, to_fl, time_descent=None, planned_avg_speed_kt=None):
        ...
        """
        return self._avg_climb_descent_ff(self.fl_climb_descent, self.descent_ff, from_fl, to_fl)

    def _avg_climb_descent_ff(self, flv, ffv, from_fl, to_fl):
        """
    ...
    def _set_climb_fuel_flow_detailed_rate_performances(self, fl, mass, fuel, rocd, gamma, tas):
        """
        Add the fl, mass, fuel, roc, gamma and tas arrays in the flight performance object
        These values come from the ptf table.
        In this case the values are more detailed as different fuel flows and rocd are
        considered as a function of altitude and mass. This allows us to interpolate the
        ...
        detailed data are used only for rocd/gamma values.
    ...
    def _interpolate_climb_performances(self, fl, mass, how="linear"):
        """
        ...
        the type of interpolation can be selected with how.

        It uses the table of performances set by set_climb_fuel_flow_detailed_rate_performances
    ...
    def _set_descent_fuel_flow_detailed_rate_performances(self, fl, mass, fuel, rocd, gamma, tas):
        """
        Add the fl, mass, fuel, roc, gamma and tas arrays in the flight performance object
        These values come from the ptf table.
        In this case the values are more detailed as different fuel flows and rocd are
    ...
    def _interpolate_descent_performances(self, fl, mass, how="linear"):
        """
        Using the data from the ptf table interpolates the climb performances
        (fuel, rocd, gamma and tas) at a given altitude (fl) and weight (mass)
    ...
    def _set_detailed_mach_nominal(self, fl, mass, m):
        """
        Add the fl, mass, m arrays in the flight performance object
        These values come from the ptd table.
        This is to have the nominal Mach as a function of altitude and mass. This is
        useful for low altitudes, i.e. <FL360 as the nominal Mach is lower than the one
        ...
        """
        self.mach_detailed_nominal = {'fl': fl, 'mass': mass, 'm': m, 'fl_mass_points': np.array((fl, mass)).T}

    def _interpolate_cruise_nominal_mach(self, fl, mass, how="linear"):
        """
        Using the data from the ptd table interpolates the nominal mach
        at a given altitude (fl) and weight (mass)
        the type of interpolation can be selected with how.

        It uses the table of performances set by set_descent_fuel_flow_detailed_rate_performances
        ...
        """
        try:
            m = griddata(self.mach_detailed_nominal.get('fl_mass_points'),
                         self.mach_detailed_nominal.get('m'), (fl, mass), method=how)

        except:
            m = interp1d(self.mach_detailed_nominal.get('fl'),
                         self.mach_detailed_nominal.get('m'), kind=how)(fl)

        return m

    def _trajectory_segment_descent_estimation_from_lnd(self, fl_0, weight_lnd, fl_1=0, d_fl=10):
        """
        ...
        fl_act = fl_1
        while fl_act < fl_0:
            perf_int = self._interpolate_descent_performances(fl_act + 10, weight)
    ...
    def _trajectory_segment_climb_estimation_from_toc(self, fl_1, weight_1, fl_0=0, d_fl=10):
        """
        ...
        Args:
            ...
            fl_0: lower altitude (ft/100), start of climb altitude (by default 0)
            d_fl: (ft/100), increment in altitude use to estimate the trajectory (Delta_fl),
                        by default: 10
        ...
        while fl_act > fl_0:
            perf_int = self._interpolate_climb_performances(fl_act - 10, weight)
            if (np.isnan(perf_int.get('fuel'))):
    ...
    def _trajectory_segment_climb_estimation_from_to(self, fl_1, weight_0, fl_0=0, d_fl=10):
        """
        Compute the trajectory climb trajectory between fl_0 and fl_1 considering
        ...
        while fl_act < fl_1:
            perf_int = self._interpolate_climb_performances(fl_act, weight)
            if (np.isnan(perf_int.get('fuel'))):
    ...
    def _trajectory_segment_cruise_estimation(self, fl_cruise, weight_1, cruise_distance,
        ...
        if (not use_mref) and (fl_cruise < 360):
            try:
                m = self._interpolate_cruise_nominal_mach(fl_cruise, weight)
                m = round(m + 0, 3)
            except:
                m = self.m_nom
        else:
    ...
    def _trajectory_cruise_estimation_with_fl_change(self, fl_1, weight_1, cruise_distance,
                                                     use_mref=False, max_steps=10000, min_d_dist=1,
        ...
        The cruise is estimated considering that the final fl used is fl_1 and
        ...
        if (not use_mref) and (fl_used < 360):
            try:
                m = self._interpolate_cruise_nominal_mach(fl_used, weight)
                m = round(m + 0, 3)
            except:
                m = self.m_nom
        else:
        ...
        while (traj.distance + dist_cruise_segment + error) < cruise_distance:
            ...
            if max_climbs > 0:
                sr = self._compute_sr(fl_used, weight, m)
                sr_lower = self._compute_sr(fl_lower, weight, m)
            ...
            while ((max_climbs > 0) and
                ...
                if dist_cruise_segment > 0:
                    ...
                    traj_segment = tr.TrajectorySegment(fl_used, fl_used, dist_cruise_segment, time_segment,
                                                        fuel_segment, weight, weight_1, "cruise",
                                                        avg_wind=avg_cruise_wind)
                    traj.add_front_trajectory_segment(traj_segment)
                ...
                c_segment = self._trajectory_segment_climb_estimation_from_toc(fl_used, weight, fl_0=fl_lower)
    ...
    @staticmethod
    def _error_extra_cruise(x, a, weight_1, distance_to_cover, fl_cruise, m, avg_cruise_wind=0):
        """
        ...
        ce = a._trajectory_segment_cruise_estimation(fl_cruise, weight_1, x, m, avg_cruise_wind)
        if ce.status != 0:
        ...
        climb_est = a._trajectory_segment_climb_estimation_from_toc(fl_cruise, weight)
    ...
    @staticmethod
    def _error_climb_weight_from_toc(weight, a, fl):
        """
        ...
        """
        cts = a._trajectory_segment_climb_estimation_from_toc(fl, weight)
        if cts.fl_0 > 0:
            # return cts.fl_0
    ...
    def _estimate_trajectory(self, weight_landing, fp_distance, fl,
        ...
        if force_descent:
            ...
            perf_int = self._interpolate_descent_performances((fl / 2), self.oew + 0.8 * self.mpl)
            rocd = perf_int.get('rocd') / 100
            time = fl / rocd
            ...
            descent_est = self._trajectory_segment_descent_estimation_from_lnd(fl, weight_landing)
        ...
        res = minimize_scalar(self._error_climb_weight_from_toc, args=(self, fl),
                              bounds=(self.oew * 1.2, self.mtow), method='bounded')

        max_weight_at_toc = res.x

        max_climb_est = self._trajectory_segment_climb_estimation_from_toc(fl, max_weight_at_toc)
        ...
        if min_cruise_dist < 0:
            if fl > 50 and try_lower_fl > 0:
                ...
                traj = self._estimate_trajectory(weight_landing, fp_distance, fl - 10,
        ...
        # ESTIMATE CRUISE
        trajectory_cruise = self._trajectory_cruise_estimation_with_fl_change(fl,
        ...
        min_climb_est = self._trajectory_segment_climb_estimation_from_toc(fl, (self.oew * 1.2))

        if ((not use_mref) and (trajectory_cruise.trajectory_segments[0].fl_0 < 360)):
            try:
                m = self._interpolate_cruise_nominal_mach(trajectory_cruise.trajectory_segments[0].fl_0,
        ...
        res = minimize_scalar(self._error_extra_cruise, args=(self,
                                                              weight_toc,
                                                              distance_to_airport, fl, m),
                              bounds=(0, (max_climb_est.distance - min_climb_est.distance)),
                              method='bounded')

        cruise_est = self._trajectory_segment_cruise_estimation(fl, weight_toc, res.x, use_mref,
                                                                avg_cruise_wind=cruise_wind)
        ...
        # ESTIMATE CLIMB
        climb_est = self._trajectory_segment_climb_estimation_from_toc(fl, weight_toc)
    ...
    def _at_gate_costs_per_minute(self):
        '''
    ...
    def _en_route_costs_per_minute(self):
        '''
        This is not BADA but from estimations done in the past (CC project).
        '''
        try:
    ...
    def _arrival_costs_per_minute(self):
        '''
        This is not BADA but from estimations done in the past (CC project).
        '''
        try:
